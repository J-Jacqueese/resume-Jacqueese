...
## 十二、Agent 高频场景题 · 系统设计篇（完整 Q&A）

> 来源：AI_Agent高频场景题_系统设计篇_面试口述版.html · 含 Coupang 特供题 & 万能答题模板

---

### 一、Agent 核心架构与运行机制（场景理解）

#### Q1：Agent 的 ReAct 模式是怎么工作的？和 Chain-of-Thought 有什么区别？【高频】
...
#### Q2：Function Calling、Tool Calling、MCP 协议三者有什么区别？【高频】
...
#### Q3：Agent 记忆模块是怎么实现的？短期记忆和长期记忆有什么区别？【高频】
...
#### Q4：Agent 工具调用失败了怎么办？如何设计错误恢复机制？【高频】
...
#### Q5：多 Agent 协作有哪几种模式？各自适用什么场景？【高频】
...
#### Q9：如何设计一个 Agent 编排平台，让非技术人员也能配置 Agent 流程？【设计题】
...
#### Q10：如何设计一个 RAG 与 Agent 结合的系统？【设计题】
...
#### Q15：如何降低 Agent 的 Token 消耗和推理成本？【高频】
...
#### Q17：如何设计一个可以自动处理用户工单的全流程 Agent？【设计题】
...
#### Q18：如何设计一个 Agent，让它能持续学习新知识而不是每次都重新训练？【设计题】
...
#### Q19：如何设计一个多模态 Agent（比如能做图文分析的客服）？【设计题】
...
#### Q20：如何设计 Agent 的可观测性体系？出了线上问题怎么排查？【高频】
...
#### Q21：如果让你从头设计一个企业级 Agent 应用，你的整体技术方案是什么？【高频】
...
#### Q22：如何设计一个实时订单履约系统（Real-time Order Fulfillment System）？【Coupang特供】
...
#### Q23：如何设计一个实时库存管理系统，防止超卖和库存不一致？【Coupang特供】
...
#### Q25：如何设计一个最后一公里配送系统（Last-Mile Delivery）？【Coupang特供】
...
#### Q26：万能系统设计答题模板 —— 任何 "Design a..." 都能用的四步框架【极高频】
...
#### Q27：Agent 系统设计万能答题模板 —— "Design an AI Agent for XX" 专用框架【极高频】
...
**一面：基础与八股文（40-50 分钟）** —— 大模型基础知识、Agent 核心概念（ReAct、工具调用、Memory）、Prompt/Context Engineering、框架使用经验、基础编程题

**二面：综合素养与分析能力（50-60 分钟）** —— 项目深度追问（动机、改进点、实际收益）、架构设计能力（Multi-Agent 系统设计）、问题解决能力、工程化细节

**三面：思维深度与职业规划（40-50 分钟）** —— 开放性问题、系统设计、产品思维、职业规划、文化匹配

### 13.2 项目深度的四个层次

1. **做了什么（能说清楚）** —— 80% 的候选人能达到
2. **怎么做的（能分析）** —— 约 50% 能达到
3. **为什么这样做（能权衡）** —— 约 20% 能达到
4. **还能怎么优化（能迭代）** —— 不到 10% 能达到

**STAR 法则：** Situation（背景）→ Task（职责）→ Action（行动，重点！）→ Result（结果，量化！）

### 13.3 Agent 开发六大常见问题

1. **幻觉问题：** RAG 引入真实数据源 + CoT 显式推理 + 外部验证 + 置信度阈值
2. **复读机问题：** 重复惩罚参数 + 最大 Token 限制 + Prompt 约束
3. **上下文溢出：** 滑动窗口 + 对话摘要 + 分层记忆 + 多 Agent 拆分
4. **工具调用失败：** 异常捕获 + 重试机制 + 降级策略 + 优雅失败
5. **死循环：** 最大步数限制 + 状态去重 + 进度监控 + 自检 Prompt
6. **成本失控：** 缓存机制 + 模型分级 + Token 预算 + 调用日志监控

### 13.4 面试准备学习路径

**第一阶段：基础夯实（2-3 周）** —— 理解 LLM 基础、掌握 Prompt Engineering、跑通 Agent demo、理解 ReAct 模式

**第二阶段：框架精通（2-3 周）** —— 深入学习一个主流框架、实现 RAG 系统、实现 Memory 模块、尝试工具调用

**第三阶段：Multi-Agent（2-3 周）** —— 理解 Multi-Agent 架构、跑通多 Agent demo、学习 MCP/A2A 协议、理解沙箱机制

**第四阶段：生产实践（持续）** —— Context Engineering、评估方法、性能优化（缓存/降级/限流）、监控告警

### 13.5 面试冲刺要点

1. ReAct / Plan-and-Solve / ToT / Reflection 是 Agent 设计的四大核心模式，务必理解适用场景和 trade-off
2. Multi-Agent 系统的核心是协作和协调，包括通信协议、冲突解决、知识共享
3. MCP 和 A2A 协议是 2025 年的新兴考点
4. Context Engineering 代表了提示工程的进化方向，要展示"系统思维"
5. Memory 机制是 Agent 的核心组件，需理解短期/长期记忆的区别、向量检索的使用场景
6. 沙箱安全性是生产级 Agent 系统的必备知识
7. 框架选择要根据团队技术栈和项目规模决定，关键是"合适"
8. 项目深度是区分候选人的关键，用 STAR 法则清晰描述，展示"思考过什么、解决过什么、反思过什么"
9. 工程化能力在二面中很重要：异常处理、性能优化、监控告警
10. Agent 开发的本质是"让 AI 能够行动"，而不只是"让 AI 能够说话"——这是 AI 从"工具"到"助手"再到"代理"的关键一步

### 13.6 未来趋势

1. **Agent 能力持续增强：** 更强的推理能力、更长的上下文、多模态支持
2. **协议标准化（MCP/A2A）：** AI Agent 领域的"USB 接口"正在形成
3. **自主性与安全性的平衡：** 沙箱技术持续演进、可解释性增强、人机协作
4. **专业化和分层：** 底层基础设施 → 中层平台服务 → 上层行业应用
5. **评估成为瓶颈：** 任务成功率评估、多维度质量评估、A/B 测试复杂性
6. **边缘 Agent：** 隐私敏感场景、实时性要求、离线能力

---

## 十四、DeepSeek / BERT / Transformer 基础

### 14.1 DeepSeek

DeepSeek（深度求索）是国内重要的开源大模型路线之一。系列包括 V2/V3、R1（强推理）等。工程亮点：MoE（混合专家）降低推理算力、MLA 等注意力优化、FP8 等训练与推理工程优化。R1 类适合复杂推理，通用对话可用 V 系列。

### 14.2 BERT 与 Transformer

- **Transformer：** 基于 Self-Attention，并行编码序列，Encoder-Decoder 结构，是现代 LLM 的基础组件
- **BERT：** Encoder-only，双向上下文建模，预训练任务 MLM（掩码语言模型）+ NSP。擅长理解、分类、向量表示。不原生自回归生成长文本
- **对比 GPT 类：** GPT 多为 Decoder-only，生成强；BERT 偏判别与表征，常用于 Embedding、检索、分类
- **一句话：** Transformer 是架构，BERT 是该架构上的一种预训练理解模型
...
- **vLLM：** PagedAttention 实现高效 KV Cache 管理，吞吐量提升显著
- **TGI（Text Generation Inference）：** HuggingFace 出品，支持张量并行、连续批处理
- **TensorRT-LLM：** NVIDIA 推理优化，支持 INT8/INT4 量化
- **SGLang：** 结构化生成语言运行时，RadixAttention 前缀缓存

### 15.2 核心优化技术

- **量化：** INT8/INT4 量化降低显存占用和计算量
- **连续批处理（Continuous Batching）：** 动态合并请求提升 GPU 利用率
- **KV Cache 优化：** PagedAttention、Multi-Query Attention、Grouped-Query Attention
- **推测解码（Speculative Decoding）：** 用小模型快速草稿，大模型并行验证
