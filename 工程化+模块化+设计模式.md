...
<img src="%E5%89%8D%E7%AB%AF%E9%9D%A2%E7%BB%8F.assets/Image%20(28).png" alt="Image.png" style="zoom: 33%;" />
...
treeshaking: 去除没用到的代码优化策略,基于ESmodule的静态结构分析,在编译阶段会把没import引入的函数删掉
...
- **构建工具**
    ...
    - Webpack
        ...
        - **模式**：需设为 `production`，该模式默认开启 `TerserPlugin`，实现 Tree - shaking 关键。否则可能失效。
        - **`optimization.minimize`**：`production` 模式下，若设为 `false`，禁用代码压缩与 Tree - shaking，因其依赖压缩移除未用模块，应设为 `true`。
        - **`babel - loader`**：Babel 转 ES6 为 CommonJS 破坏 Tree - shaking 静态分析，用 `@babel/plugin - transform - runtime` 插件解决。

        Rollup

        - **输出格式**：`es` 和 `iife` 格式支持 Tree - shaking，`cjs` 不支持，配置选合适格式。
        - **插件**：部分非标准插件处理模块导入导出，破坏静态结构，干扰 Tree - shaking，需检查兼容性。

    

- **代码结构**
    - **副作用代码**：模块含修改全局变量、`console.log` 等副作用代码，即便部分函数未用，整个模块也难被 Tree - shaking 移除，应避免或封装。
    - **复杂导出**：如 `export default { func1, func2 }` 结构，难判断函数使用情况，简单命名导出 `export function func1() {}` 更利于 Tree - shaking。

- 第三方库
    - **构建方式**：用 CommonJS 且无 ES6 模块版本库，不支持 Tree - shaking，可找替代库或查文档有无相关构建选项。
    - **导入方式**：支持 Tree - shaking 的库，要精准导入，如 `import { specificFunc } from 'lib'`，按文档推荐方式，否则不生效。
